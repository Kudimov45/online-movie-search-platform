...
import Refresh from '../../assets/svg/refresh-icon.svg?react';
...
export interface RandomMovieProps {
  movie: Movie
  refetch: UseQueryResult<Movie, unknown>['refetch'];
}

export const CardRandomMovie: FC<RandomMovieProps> =  ({movie, refetch}) => {
        ...
        <div className="card--movie">
            <div className="card__wrapper">
                ...
                <button className="card__link-trailer btn btn--submit" onClick={() => dispatch(openVideoPlayerModal({ videoUrl: movie.trailerUrl, title: movie.title }))}>Трейлер</button>
                    <Link className="card__link-about" to={`movie/${movie.id}`}>О фильме</Link>
                    {isFavorite ? 
                    ...
                    <button className="card__link-new btn" type="button" onClick={()=> refetch()}>{<Refresh/>}</button>
                </div>
            </div>
